...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (String(nim) == "G.231.21.0215") {
    Serial.println(messages);
    if (String(messages) == "D1=1") { digitalWrite(D1, HIGH); }
    if (String(messages) == "D2=1") { digitalWrite(D2, HIGH); }
    ...
    if (String(messages) == "D1=0") { digitalWrite(D1, LOW); }
    if (String(messages) == "D2=0") { digitalWrite(D2, LOW); }
...
void loop() {
  ...
  // Buat pesan JSON
  String jsonMessage = "{ \"nim\" : \"G.231.21.0215\", \"suhu\" : " + String(t) + ", \"kelembapan\" : " + String(h) + " }";
